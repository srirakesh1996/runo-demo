...
function submitForm(formId, formData, formToken) {
  ...
  if (typeof clevertap !== "undefined") {
    ...
    const profilePayload = { Name: formData.your_name || "", Email: formData.your_email || "", Identity: formData.your_email || fixedPhone || "anonymous_user", Phone: fixedPhone, "Company Name": formData.your_company || "", license_count: formData["custom_Sales/Calling Team Size"] || "", KnowSource: formData["custom_We entered source"] || "", "MSG-whatsapp": whatsappOptIn };
    clevertap.onUserLogin.push({ Site: profilePayload });
  }
  if (typeof clevertap !== "undefined") {
    const rawPhone = String(formData.your_phone || formData.phone || "");
    const fixedPhone = rawPhone.startsWith("+") ? rawPhone : "+" + rawPhone;
    const eventPayload = { Name: formData.your_name || "", Email: formData.your_email || "", Phone: fixedPhone, "Company Name": formData.your_company || "", noOflicenses: formData["custom_Sales/Calling Team Size"] || "", KnowSource: formData["custom_We entered source"] || "", Source: utmSource || "Website", Campaign: utmCampaign || "", "MSG-whatsapp": whatsappOptIn, Timestamp: new Date().toISOString() };
